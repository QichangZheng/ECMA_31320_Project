...
def reverse_cut_to_length(content, code_func, cut_num=4, fill_num=7):
    """
    This function is used to deal with the content that is encoded by base62.
    Input: 
        content: str
        code_func: function
        cut_num: int
        fill_num: int
    Output: str that is the mid
...
def url_to_mid(url: str):
    """
    This function is to decode the url to mid.
    3501756485200075
    """
...
def parse_time(s):
    """
    This function is to parse the time. 
    For example:
...
def parse_user_info(data):
    """
    uncode user info
    Input: dict data from json
    Output: dict user info
    """
    # base info
    user = {
        "_id": str(data['id']),
